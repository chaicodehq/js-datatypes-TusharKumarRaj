...
export function parseWhatsAppMessage(message) {
  ...
  let sender = message.slice(message.indexOf(" - ") + 3, message.indexOf(": "));
  let text = message.slice(message.indexOf(": ") + 2).trim();
  
  let textArray = text.split(" ").filter((e) => e !== "");

  let wordCount = textArray.length;

  let sentiment;
  let textChecker = text.toLowerCase();

  if (textChecker.includes("😂") || textChecker.includes(":)") || textChecker.includes("haha"))
    sentiment = "funny";

  else if (textChecker.includes("❤") || textChecker.includes("love") || textChecker.includes("pyaar"))
    sentiment = "love";
