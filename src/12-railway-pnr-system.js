...
export function processRailwayPNR(pnrData) {
  ...
  let train = pnrData.train
  let trainInfo = `Train: ${train.number} - ${train.name} | ${train.from} → ${train.to} | Class: ${pnrData.classBooked}`

  let passengers = pnrData.passengers.map((p) => {
    let statusLabel = ""
    ...
    let formattedName = p.name.padEnd(20) + "(" + p.age + "/" + p.gender + ")"

    return {formattedName, bookingStatus: p.booking, currentStatus: p.current, statusLabel, isConfirmed,}
  ...
  return {pnrFormatted, trainInfo, passengers,summary: { totalPassengers, confirmed, waiting, cancelled, rac,
           allConfirmed, anyWaiting, }, chartPrepared}
}
